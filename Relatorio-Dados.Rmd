...
for (step in 0:4) {
  ...
  ajustes_modelos %<>% bind_rows(log_multipla_BIC %$% tibble(
    ...
    AIC = aic,
    BIC = BIC(.),
    "P-valor" = ifelse(1 - pchisq((null.deviance - deviance), (df.null - df.residual)) < 0.001,
                                "< 0.001",
                                round(pvalor, 3)),
  )
)
...
probabs_preditas <- list(predict(modelo_final_mais_complexo, newdata = validacao,type = "response"),
...
tabela_acuracia$acuracia[i] <-confusionMatrix(truth_predicted$pred, validacao$X4)$overall[1]
}
...
invisible(plot(roc(validacao$X4,
...
Os resultados observados na curva ROC são condizentes com a tabela de valores da acurácia para cada modelo.
