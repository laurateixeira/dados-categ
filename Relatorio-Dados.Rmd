...
probabs_preditas <- predict(
  modelo_final_mais_complexo,
  newdata = validacao,
  type = "response"
)
...
res_padronizado_2 <- tibble(
  ...
  ggplot(aes(x=index, y=residuos)) +
  labs(title="Modelo de três parâmetros")+
  geom_point() +
  geom_hline(yintercept = 0, color="orange") +
  tema
...
res_padronizado_3 <- tibble(
  ...
  ggplot(aes(x=index, y=residuos)) +
  geom_point() +
  labs(title="Modelo de dois parâmetros") +
  geom_hline(yintercept = 0, color="orange") +
  tema
...
map_df(0:1, function(x1_dummy) {
  ...
  ggplot() +
  geom_path(aes(x=x3, y=prob, color=grupo, group=grupo))
```
\newpage
