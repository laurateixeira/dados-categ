...
Na segunda etapa do estudo considera-se, além da variável nível de fosfatase ácida, as outras variáveis pré-operatórias. Assim, para a seleção das variáveis do modelo utilizamos o método stepwise considerando os critérios AIC e BIC.

```{r}
log_multipla <- glm(X4 ~ X3 + factor(X1) + factor(X2), family=binomial, data=treinamento)
```

```{r}
library(MASS)
log_multipla_AIC <- log_multipla %>% stepAIC(trace = TRUE, direction="both")
summary(log_multipla_AIC)
```

```{r}
log_multipla_BIC <- log_multipla %>% stepAIC(trace = TRUE, direction="both", k=log(51))
summary(log_multipla_BIC)
```

Em ambos casos o modelo final selecionado considera todas as variáveis. No entanto, observe como os resultados do AIC e do BIC para o modelo com duas variáveis (resultado da radiografia e estágio do tumor) também apresenta valores relativamente baixos.
...
Os modelos ajustados são descritos a seguir.

$$\text{logito}[\hat{\pi}(x)] = -2.30638 + 0.03035x_1 + 2.80120x_2 + 3.13819x_3$$
$$\text{logito}[\hat{\pi}(x)] = -2.5888 + 2.7833x_2 + 2.5243x_3$$
Assume-se que não existe interação entre nível de fofatase ácida e estágio do tumor ou nível de fofatase ácida e resultado da radiografia ou estágio do tumor e resultado da radiografia.
...
<!-- Testando deviance PARCIAL para modelo mais simples -->

Dado que o modelo mais complexo se ajusta, deseja-se testar que o modelo mais simples se ajusta.

```{r}
G_2 <- modelo_final_simples$deviance - modelo_final_complexo$deviance
1-pchisq(G_2,1)
```
...
invisible(plot(roc(treinamento$X4,
               ...
               print.auc = T,
               col = "red", 
               main = "Curvas ROC: Modelo de 4 parâmetros (red) vs. Modelo de 3 parâmetros"))
